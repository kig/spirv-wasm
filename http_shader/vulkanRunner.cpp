...
class ComputeApplication
{
  private:
    ...
    uint32_t heapSize = 1024;

    uint32_t workSize[3] = {32, 1, 1};
    char *input;
    char *heap;

    const char *programFileName;

    uint32_t requestCount = workSize[0] * workSize[1] * workSize[2] * 2048 * 8;
  ...
  public:
    ...
    void createBuffer()
    {
        ...
        createAndAllocateBuffer(&heapBuffer, heapBufferSize, &heapBufferMemory);
    }
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        app.run(argc > 1 ? argv[1] : "httpd.spv");
    }
